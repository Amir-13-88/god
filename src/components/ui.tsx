...
export function BoxBadge({ s }: { s: SavedWord }) {
  ...
  return (
    <span className="text-[10px] font-bold rounded-full px-2 py-0.5 text-white shrink-0" style={{ backgroundColor: BOX_COLORS[idx] }}>
      {BOX_NAMES[idx]}
    </span>
  );
}
...
export function SaveButton({ entry, big }: { entry: Entry; big?: boolean }) {
  ...
  return (
    <button
      onClick={(e) => { e.stopPropagation(); toggleSave(entry); }}
      title={saved ? "حذف از مجموعه‌ی من" : "افزودن به مجموعه‌ی من"}
      className={`shrink-0 grid place-items-center rounded-full border-2 transition-all active:scale-90 ${big ? "w-11 h-11" : "w-9 h-9"} ${
        saved
...
export function WordRow({ entry, saved, delay = 0, trailing }: { entry: Entry; saved?: SavedWord; delay?: number; trailing?: ReactNode }) {
    ...
    >
      <div className="min-w-0 flex-1">
        <div className="flex items-baseline gap-2 flex-wrap">
          ...
          {entry.pos && <span className="ltr-i text-[11px] text-mute bg-oxford-soft rounded px-1.5 py-px font-latin">{entry.pos}</span>}
      ...
      <span
        ...
        tabIndex={0}
        title="شنیدن تلفظ"
        onClick={(ev) => { ev.stopPropagation(); speak(entry.w); }}
        onKeyDown={(ev) => { if (ev.key === "Enter") { ev.stopPropagation(); speak(entry.w); } }}
...
export function StatCard({ label, value, icon, tone }: { label: string; value: number; icon: string; tone: string }) {
  return (
    <div className="bg-card border-2 border-line rounded-xl p-3.5 text-center">
      ...
      <div className="font-display text-2xl leading-7" style={{ color: tone }}>{faNum(value)}</div>
...
export function Card({ title, icon, tone = "#0b2e52", children }: { title: string; icon: string; tone?: string; children: ReactNode }) {
  return (
    <section className="anim-rise bg-card border-2 border-line rounded-2xl p-4 sm:p-5">
      <div className="flex items-center gap-2.5 mb-4">
        ...
        <h2 className="font-display text-xl" style={{ color: tone }}>{title}</h2>
      </div>
      {children}
    </section>
  );
